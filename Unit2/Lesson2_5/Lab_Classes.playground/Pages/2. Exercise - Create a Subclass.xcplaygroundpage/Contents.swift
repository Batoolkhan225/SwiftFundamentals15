...
print("\(destroyer.name) is at position \(destroyer.position)")

destroyer.moveRight()
print("\(destroyer.name) moved to position \(destroyer.position)")
...
// print(falcon.weapon) ❌ ERROR: 'Spaceship' has no member 'weapon'
...
extension Fighter {
    func fire() {
        if remainingFirePower > 0 {
            ...
            print("\(name) fired! Remaining firepower: \(remainingFirePower)")
        } else {
            print("You have no more firepower.")
        }
    }
}
destroyer.fire() // Output: Destroyer fired! Remaining firepower: 9
destroyer.fire() // Output: Destroyer fired! Remaining firepower: 8
destroyer.fire() // Output: Destroyer fired! Remaining firepower: 7
/*:
